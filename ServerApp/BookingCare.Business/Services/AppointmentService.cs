...
namespace BookingCare.Business.Services
{
    public class AppointmentService : BaseService<Appointment>, IAppointmentService
    {
        ...
        public async Task<bool> ManageAppointmentAsync(int appointmentId, string status, int userId)
        {
            var appointment = await GetByIdAsync(appointmentId);
            if (appointment == null) return false;

            // Check if user is doctor or patient
            if (status == "Cancelled" && appointment.PatientId != userId)
                throw new Exception("Only patient can cancel appointment");
            if ((status == "Accepted" || status == "Declined") && appointment.DoctorId != userId)
