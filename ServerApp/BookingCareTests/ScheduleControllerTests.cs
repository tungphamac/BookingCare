...
namespace BookingCareTests
{
    ...
    public class ScheduleControllerTests
    {
        ...
        public async Task CreateSchedule_ReturnsOk_WhenSuccessful()
        {
            ...
            int createdScheduleId = 123;

            _mockScheduleService.Setup(s => s.CreateScheduleAsync(scheduleDto, doctorId))
                                .ReturnsAsync(createdScheduleId);

            // Act
            var result = await _controller.CreateSchedule(doctorId, scheduleDto);

            // Assert
            Assert.IsInstanceOf<OkObjectResult>(result);
            var okResult = result as OkObjectResult;

            JObject response = JObject.FromObject(okResult!.Value!);

            Assert.AreEqual("Schedule created successfully.", response["Message"]!.ToString());
            Assert.AreEqual(createdScheduleId.ToString(), response["ScheduleId"]!.ToString());
        }

        [Test]
        public async Task CreateSchedule_ReturnsServerError_WhenExceptionThrown()
        {
            // Arrange
            int doctorId = 1;
            var scheduleDto = new CreateScheduleDto();
            ...
            Assert.AreEqual(500, objectResult!.StatusCode);
            Assert.AreEqual("An error occurred while creating the schedule.", objectResult.Value);
        }

        [Test]
        public async Task DeleteSchedule_ReturnsOk_WhenDeletedSuccessfully()
        {
            // Arrange
            int scheduleId = 1;
            ...
            JObject response = JObject.FromObject(okResult!.Value!);
            Assert.AreEqual("Schedule deleted successfully.", response["Message"]!.ToString());
        }

        [Test]
        public async Task DeleteSchedule_ReturnsNotFound_WhenScheduleNotFound()
        {
            // Arrange
            int scheduleId = 999;
            _mockScheduleService.Setup(s => s.DeleteScheduleAsync(scheduleId))
            ...
            JObject response = JObject.FromObject(notFound!.Value!);
            Assert.AreEqual($"Schedule with ID {scheduleId} not found.", response["Message"]!.ToString());
        }
